...
- **修复广州GF308 滴滴行程单 A 解析丢失第 2 笔 + 起终点错位**：用户报告 `D:/工作PRO/发票聚合/2026/重庆+成都局方/广州GF308/滴滴出行行程报销单A.pdf`（申请日期 2026-07-08，行程 2026-06-25~26，2 笔 52.70 元）当前只解析出 1 笔
  - **根因（双重 bug）**：
    - **车型词缺失**：完整 CAR_ALT 缺少 `滴滴轻\s*享`（也未含 `轻享` 单词），导致笔 2「2 滴滴轻 享」无法被 ENTRY_RE 锚定 → 该笔并入笔 1 的 seg，金额混入笔 1 `to` 字段
    - **单 | 切分失效**：广州新版式把起点换行拆成两段并用 | 衔接（"白云机场-T3-P12停车楼二层-上车点|白云机场-T3-P12停车楼"），后面接 wrap 续行"二层-上车点"+终点带括号地址"悦云酒店(...)"。原 v119 单 | 切分逻辑错误地把起点切碎、终点吞并入 from
  - **修复**：
    - **CAR_ALT 扩充**：`滴滴轻\s*享|滴滴豪\s*华|滴滴舒\s*适|滴滴专\s*车`（滴滴前缀词系，同 `滴滴特\s*快` 模式）；并独立加入 `轻享` 单词（与 `特惠|优享` 同级）
    - **单 | 切分重写**：起点取 `|` 之前；终点从 `|` 后第一个 `(` 向前找最近空格作为店名起点（适配"重复起点 + 括号地址终点"wrap 模式）；无括号时按 v119 兜底按 | 后第一个空格切
  - **验证**：
    - **广州GF308 A.pdf**：1 笔 → **2 笔**，合计 22.9 + 29.8 = **52.70 元**，与票面"共2笔行程，合计52.70元"完全吻合
    - **回归**：上海加混哪 C/A/B/T3 三类行程单全部正常（C.pdf 9 笔 151.36 元、A 17.60、B 19.80、T3 24.58），未被破坏
